...
class P extends \PhpParser\NodeVisitorAbstract
{
    ...
    public function enterNode(\PhpParser\Node $node)
    {
        if ($this->isTrait && count($this->structure) > 1 && $node instanceof \PhpParser\Node\Param)
        {
            ...
            $latest = [
                'name' => $node->var->name,
                'type' => isset($node->type->parts) && count($node->type->parts) > 0 ? implode('\\', $node->type->parts) : $node->type,
            ];
            ...
            foreach ($reflectionMethod->getParameters() as $idx => $parameter) {
                ...
                if ($parameter->isDefaultValueAvailable()) {
                    if(is_array($parameter->getDefaultValue())) {
                        ...
                        $definition .= ' = ' . (string)$parameter->getDefaultValue();
                    }
                }
...
$directory = __DIR__.'/../../vendor/iota-php/client/src';
